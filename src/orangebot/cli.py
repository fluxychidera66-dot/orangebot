"""Command-line interface for orangebot."""
...
from orangebot import __version__
from orangebot.config import get_settings, reload_settings
from orangebot.utils.logging import setup_logging
...
def cli() -> None:
    """🍊 OrangeBot - Automated Polymarket Arbitrage Trading Bot."""
    pass
...
def run(
    ...
    console.print(f"\n[bold orange1]🍊 OrangeBot v1.0.0[/bold orange1] - Polymarket Arbitrage Bot")
    console.print(f"[dim]Free to use · 1% transparent developer fee per trade[/dim]\n")
    ...
    try:
        if realtime:
            from orangebot.bot import run_realtime_bot
            asyncio.run(run_realtime_bot())
        else:
            from orangebot.bot import run_bot
            asyncio.run(run_bot())
...
def scan() -> None:
    ...
    async def _scan() -> None:
        from orangebot.analyzer.arbitrage import ArbitrageAnalyzer
        from orangebot.scanner.market_scanner import MarketScanner
...
def markets(limit: int) -> None:
    ...
    async def _markets() -> None:
        from orangebot.api.gamma import GammaClient
...
def crossplatform(
    ...
    async def _run() -> None:
        from orangebot.scanner.crossplatform_scanner import CrossPlatformScanner
...
def kalshi_test() -> None:
    ...
    async def _test() -> None:
        from orangebot.api.kalshi import KalshiClient
...
def status() -> None:
    ...
    async def _status() -> None:
        ...
        console.print("\n[bold]orangebot Bot Status[/bold]\n")
        ...
        # Record snapshot
        from orangebot.tracking.portfolio import BalanceSnapshot
...
def redeem() -> None:
    ...
    async def _redeem() -> None:
        from orangebot.executor.redemption import get_redeemable_positions, redeem_all_positions
...
def positions() -> None:
    ...
    async def _positions() -> None:
        from orangebot.executor.redemption import get_redeemable_positions
        import httpx
        ...
        if redeemable:
            ...
            console.print("[dim]Run 'orangebot redeem' to claim these positions[/dim]")
...
def dashboard(host: str, port: Optional[int]) -> None:
    ...
    console.print(f"\n[bold]Starting orangebot Dashboard[/bold]")
    ...
    from orangebot.dashboard import run_dashboard
