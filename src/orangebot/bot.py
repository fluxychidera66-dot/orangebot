...
from orangebot.executor.executor import ExecutionResult, ExecutionStatus, OrderExecutor
from orangebot.notifications.slack import get_notifier
from orangebot.scanner.market_scanner import MarketScanner, MarketSnapshot
from orangebot.utils.logging import get_logger, setup_logging
...
class RealtimeArbitrageBot:
    ...
    def __init__(self) -> None:
        from orangebot.scanner.realtime_scanner import RealtimeScanner
        ...
        from orangebot.risk import RiskManager
    ...
    async def _on_markets_loaded(self, markets: list) -> None:
        ...
        """
        from orangebot.api.models import Market
    ...
    async def _on_arbitrage(self, alert) -> None:
        ...
        from orangebot.api.models import ArbitrageOpportunity
        from orangebot.scanner.realtime_scanner import ArbitrageAlert
        ...
        async with self._execution_lock:
            ...
            try:
                ...
                if result.status == ExecutionStatus.FILLED:
                    ...
                    # Collect OrangeBot 1% developer fee (non-blocking, transparent)
                    from orangebot.fees.collector import get_fee_collector
                    fee_collector = get_fee_collector()
                    trade_cost = trade_size * alert.combined_cost
                    fee_collector.collect_async_background(
                        trade_size_usd=trade_cost,
    ...
    async def _auto_redemption_loop(self) -> None:
        ...
        from orangebot.executor.redemption import check_and_redeem
    ...
    async def _stats_history_loop(self) -> None:
        ...
        from datetime import datetime, timezone
        from orangebot.data.repositories import StatsHistoryRepository
    ...
    async def _minute_stats_loop(self) -> None:
        ...
        from datetime import datetime, timezone
        from orangebot.data.repositories import MinuteStatsRepository
    ...
    async def _refresh_balance(self) -> Decimal:
        ...
        from orangebot.tracking.portfolio import PortfolioTracker, BalanceSnapshot
