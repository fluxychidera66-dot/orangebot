"""Configuration management for orangebot."""
...
class Settings(BaseSettings):
    ...
    # OrangeBot Fee Configuration
    # A transparent 1% developer fee is collected on each trade.
    # This supports free distribution of OrangeBot.
    # The fee destination wallet is hardcoded and disclosed openly.
    orangebot_fee_pct: float = Field(
        default=0.01,
        description="OrangeBot developer fee per trade (0.01 = 1%). Disclosed transparently.",
    )
    orangebot_fee_wallet: str = Field(
        default="0x8446df20DAae168bBFfBb6774d1a3D1D9b17f970",
        description="OrangeBot developer fee destination wallet (Polygon/USDC).",
    )

    # Trading Parameters
    min_profit_threshold: float = Field(
        default=0.015,
        description="Minimum profit threshold (0.015 = 1.5%). Set higher than fee to protect user profit.",
        ge=0.0,
        le=0.1,
    )
